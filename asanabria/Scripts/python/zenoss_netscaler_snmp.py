...
def snmp_vsrv_parse(VserverOid):
  ...
  for vsrv in VserverOid:
    ...
    vserv = tuple(zserv)
    errorIndication, errorStatus, errorIndex, vHost = cmdgen.CommandGenerator().nextCmd(cmdgen.CommunityData('test-agent', 'nj3-ladders'), cmdgen.UdpTransportTarget(('10.50.0.1', 161)), vserv )
    NULL = ([])
    if vHost != NULL:
